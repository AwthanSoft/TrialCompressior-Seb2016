...
namespace BWT.AsInt
{
    class BWTintImplementation
    {
        public void bwt_encode(int[] buf_in, int[] buf_out, int size, ref int primary_index)
        {
            int[] indices = new int[size];
            for (int i = 0; i < size; i++)
                indices[i] = i;

            Array.Sort(indices, 0, size, new BWTintComparator(buf_in, size));
        ...
        public void bwt_decode(int[] buf_encoded, int[] buf_decoded, int size, int primary_index)
        {
            int[] F = new int[size];
    ...
    class BWTintComparator : IComparer<int>
    {
        private int[] rotlexcmp_buf = null;
        private int rottexcmp_bufsize = 0;

        public BWTintComparator(int[] array, int size)
        {
            rotlexcmp_buf = array;
